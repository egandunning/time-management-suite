...
public class GTD {
	...
	lists = new ArrayList<>();
	lists.addAll(Arrays.asList(new Pair<>("inList", inList), new Pair<>("nextActionsList", nextActionsList),
		new Pair<>("waitingForList", waitingForList), new Pair<>("projectsList", projectsList),
	...
	for (Pair<String, VBox> list : lists) {
	    ...
	    List<GTDText> storedLists = Serializer.getInstance().readGTDList(list.getKey());
	    ...
	    list.getValue().setOnDragDropped(new DragDroppedHandler<DragEvent>(list.getValue()));

	    // list changed event listener
	    list.getValue().getChildren().addListener(new ListChangedHandler<Node>(list.getKey()));
	}
    ...
    /**
     * Event handler for saving list to file and adding event listeners to GTD list
     * items when list is updated.
     */
     ...
     * 
     * @param <T>
     *            the event type - should be DragEvent
     */
    ...
    /**
     * Class for handling drop events - needed to move items from one layout to
     * another
     * 
     * @param <T>
     *            the event type - should be DragEvent
	...
	public void handle(Event event) {
		...
		layout.getChildren().add(ListItemElement
			.generate((GTDListItem) (((DragEvent) event).getDragboard().getContent(GTD_LIST_ITEM))));
	    }
    ...
    /**
     * Add an idea to a list. List types are defined in data.GTDLists.java Helper
     * method for new idea event handler.
     * 
     * @param idea
     *            the idea to record
     * @param list
     *            the list to record idea in
     */
