...
public class GTDNewIdea {
	...
	list = new ComboBox<>(FXCollections.observableList(GTDLists.getInstance().getLists()));
     ...
     * 
     * @return a Pair of strings containing the idea and the list to put the idea
     *         in, or null if the user presses cancel.
     */
	...
	do {
		...
		&& (!FutureDate.validate(datePicker.getValue()) || !TimeFormat.validate(timeField.getText())));
	...
	if (showDeadline.isSelected() && datePicker.getValue() != null) {
	    item.setDeadline(LocalDateTime.of(datePicker.getValue(), LocalTime.parse(timeField.getText())));
	}
